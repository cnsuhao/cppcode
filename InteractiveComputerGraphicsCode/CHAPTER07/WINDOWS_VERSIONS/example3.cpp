...
static void init()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(points)+sizeof(tex_coord), NULL, GL_STATIC_DRAW);
	glBufferSubData( GL_ARRAY_BUFFER, 0, sizeof(points), points );
	glBufferSubData( GL_ARRAY_BUFFER, sizeof(points), sizeof(tex_coord), tex_coord );

   loc = glGetAttribLocation(program, "vPosition");
   glEnableVertexAttribArray(loc);
   glVertexAttribPointer(loc, 4, GL_FLOAT, GL_TRUE, 0, BUFFER_OFFSET(0));

   loc2 = glGetAttribLocation(program, "texcoord");
   glEnableVertexAttribArray(loc2);
   glVertexAttribPointer(loc2, 2, GL_FLOAT, GL_TRUE, 0, BUFFER_OFFSET(sizeof(points)));
   ...
   nm[2][2] = (ctm[0][0]*ctm[1][1]-ctm[1][0]*ctm[0][1])/det;
    ...
    projection = Ortho(-1.75,1.75,-1.75,1.75,-5.5,5.5);
