...
void MyThread::TFuncOne()
{
    ...
    for (Int32 i = 0; i < 3; i++)
    {
        Console::WriteLine("TFuncOne   Waits  {0}", i.ToString());
        Monitor::Wait(MonitorObject);
        Console::WriteLine("TFuncOne   Pulses {0}", i.ToString());
...
void MyThread::TFuncTwo()
{
    ...
    for (Int32 i = 0; i < 3; i++)
    {
        Console::WriteLine("TFuncTwo   Pulses {0}", i.ToString());
        Monitor::Pulse(MonitorObject);
        Thread::Sleep(1);
        Console::WriteLine("TFuncTwo   Waits  {0}", i.ToString());
        Monitor::Wait(MonitorObject);
    }
    Monitor::Exit(MonitorObject);
