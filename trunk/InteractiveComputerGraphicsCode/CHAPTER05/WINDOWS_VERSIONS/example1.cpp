...
#define N 1  // number of subdivisions
#define M 4*3*4  // number of resulting points
...
GLfloat left= -2.0, right=2.0, top=2.0, bottom= -2.0, nearhehe= -4.0, farhehe=4.0;
...
void display()
{
     ...
     projection = Ortho(left, right, bottom, top, nearhehe, farhehe);
...
void mykey(unsigned char key, int mousex, int mousey)
{
   ...
   if(key == 'z') {nearhehe  *= 1.1; farhehe *= 1.1;}
   if(key == 'Z') {nearhehe *= 0.9; farhehe *= 0.9;}
   if(key == 'r') radius *= 2.0;
   if(key == 'R') radius *= 0.5;
   if(key == 'o') theta += dr;
   if(key == 'O') theta -= dr;
