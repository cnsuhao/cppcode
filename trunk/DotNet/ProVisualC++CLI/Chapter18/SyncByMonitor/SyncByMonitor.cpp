...
void MyThread::TFuncOne()
{
    ...
    for (Int32 i = 0; i < 3; i++)
    {
        Console::WriteLine("TFuncOne  to Waits  {0}", i.ToString());
        Monitor::Wait(MonitorObject);
        Console::WriteLine("TFuncOne  to Pulses {0}", i.ToString());
...
void MyThread::TFuncTwo()
{
    ...
    for (Int32 i = 0; i < 3; i++)
    {
        Console::WriteLine("TFuncTwo  to Pulses {0}", i.ToString());
        Monitor::Pulse(MonitorObject);
        Thread::Sleep(1);
        Console::WriteLine("TFuncTwo  to Waits  {0}", i.ToString());
        Monitor::Wait(MonitorObject);
    }
    Monitor::Exit(MonitorObject);
...
void main()
{
    ...
    {
        (gcnew Thread(
            gcnew ThreadStart(myThr1, &MyThread::TFuncThree)))->Start();
        Thread::Sleep(50);
   }
}
